...
$SCRIPTPATH/../vfr2${PGM}.py --file $SCRIPTPATH/OB_UKSH.xml.gz $OPT

echo "2nd PASS (already exists...)"
$SCRIPTPATH/../vfr2${PGM}.py --file $SCRIPTPATH/OB_UKSH.xml.gz $OPT

echo "3rd PASS (overwrite...)"
$SCRIPTPATH/../vfr2${PGM}.py --file $SCRIPTPATH/OB_UKSH.xml.gz $OPT --o

echo "4th PASS (append...)"
$SCRIPTPATH/../vfr2${PGM}.py --file $SCRIPTPATH/OB_UKSH.xml.gz $OPT --a

echo "5th PASS (geom_name...)"
$SCRIPTPATH/../vfr2${PGM}.py --file $SCRIPTPATH/OB_UKSH.xml.gz $OPT --o --geom OriginalniHranice

if [ "$PGM" = "pg" ] ; then
    echo "6th PASS (schema per file...)"
    $SCRIPTPATH/../vfr2${PGM}.py --file $SCRIPTPATH/OB_UKSH.xml.gz $OPT -s
fi
